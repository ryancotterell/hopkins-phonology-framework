...
{{/types}}
    ...
    cdef void Synchronize(Fst& ifst, MutableFst* ofst)
...
cdef extern from "<fst/script/draw.h>" namespace "fst":
    ...
    cdef cppclass SignedLogWeight(Weight):
        TropicalWeight Value1()
        LogWeight Value2()
        SignedLogWeight(float value1, float value2)
        SignedLogWeight(SignedLogWeight w)
        bint operator==(SignedLogWeight& other)
        SignedLogWeight& set_value "operator=" (SignedLogWeight& other)
        
    cdef SignedLogWeight Plus(SignedLogWeight &w1, SignedLogWeight& w2)
    cdef SignedLogWeight Times(SignedLogWeight &w1, SignedLogWeight& w2)
    cdef SignedLogWeight Divide(SignedLogWeight &w1, SignedLogWeight& w2)


    cdef SignedLogWeight SignedLogWeightZero "fst::SignedLogWeight::Zero" ()
    cdef SignedLogWeight SignedLogWeightOne "fst::SignedLogWeight::One" ()


    ctypedef Arc[SignedLogWeight] SignedLogArc



    cdef cppclass ExpectationWeight "fst::ExpectationWeight<fst::SignedLogWeight,fst::SignedLogWeight>"(Weight):
        SignedLogWeight Value1()
        SignedLogWeight Value2()
        ExpectationWeight(SignedLogWeight weight1, SignedLogWeight weight2)
        ExpectationWeight(ExpectationWeight w)
        bint operator==(ExpectationWeight& other)
        ExpectationWeight& set_value "operator=" (ExpectationWeight& other)
        
    cdef ExpectationWeight Plus(ExpectationWeight &w1, ExpectationWeight& w2)
    cdef ExpectationWeight Times(ExpectationWeight &w1, ExpectationWeight& w2)
    cdef ExpectationWeight Divide(ExpectationWeight &w1, ExpectationWeight& w2)


    cdef cppclass ExpectationArc "fst::ExpectationArc<fst::SignedLogArc,fst::SignedLogWeight>"(Weight):
        int ilabel
        int olabel
        int nextstate
        ExpectationArc(int ilabel, int olabel, ExpectationWeight& weight, int nextstate)
        ExpectationWeight weight


    cdef cppclass ExpectationVectorFst "fst::VectorFst<fst::ExpectationArc<fst::SignedLogArc,fst::SignedLogWeight> >" (MutableFst):
        ExpectationWeight Final(int s)
        void AddArc(int s, ExpectationArc &arc)
